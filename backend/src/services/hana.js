...
function connParams(){
  const p = { serverNode: `${HANA.server}:${HANA.port}`, uid: HANA.uid, pwd: HANA.pwd };
...
export async function getCardCodeByCNPJ_HANA(cnpjInput){
  const digits = normalizeCNPJNumeric(cnpjInput||''); if (digits.length!==14) return null;
  const formatted = formatCNPJMask(digits); if (!formatted) return null;

  const rowExact = await queryOne(`
    SELECT T0."CardCode" FROM CRD7 T0
    JOIN OCRD T1 ON T1."CardCode"=T0."CardCode"
    WHERE T0."TaxId0" = ? LIMIT 1`, [formatted]);
  if (rowExact?.CardCode) return rowExact.CardCode;

  const rowDigits = await queryOne(`
    SELECT T0."CardCode" FROM CRD7 T0
    JOIN OCRD T1 ON T1."CardCode"=T0."CardCode"
    WHERE REPLACE(REPLACE(REPLACE(T0."TaxId0",'.',''),'/',''),'-','') = ? LIMIT 1`, [digits]);
